...
4. If any library is not installed in the env, you MUST install it first using a shell command block, then provide the Python code in a separate block.

CRITICAL: When installing packages, provide the shell command exactly like this:
```sh
pip install matplotlib
```

After the installation command, provide your Python code in a separate code block:
```python
# Your complete Python code here
```

The code executor will execute the shell command first to install packages, then execute your Python code. Always provide complete, standalone code blocks - don't assume the executor remembers previous code.
